...
vars_num_names <- names(data)[sapply(data, is.numeric)]
# 
...
cor_matrix <- cor(data[vars_num_names], use = "complete.obs")
...
vars_binary_names <- names(data)[sapply(data, function(x) is.factor(x) && length(levels(x)) == 2)]

vars_cat_names <- names(data)[sapply(data, is.factor)]
vars_cat_names <- names(data)[sapply(data, is.logical)]

# Set the size of the plotting device
png("Correlation_plot_categorical.png", width = 800, height = 800)

# Create a contingency table for the binary variables
contingency_table <- table(data[, vars_cat_names, drop = FALSE])
